...
interface FriendRequestStatusToolbarProps<TData> {
  table: Table<TData>;
}
 ...
 */
export default function FriendRequestStatusToolbar<TData>({ table }: FriendRequestStatusToolbarProps<TData>) {
  "use no memo";
    ...
    <div className="flex items-center justify-between">
      <div className="flex flex-1 items-center space-x-2">
        <Input
          ...
          value={(table.getColumn("sentTo")?.getFilterValue() as string) ?? ""}
          onChange={(event) =>
            table.getColumn("sentTo")?.setFilterValue(event.target.value)}
